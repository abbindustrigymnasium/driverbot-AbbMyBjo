    ...
    <v-card class="elevation-12" color="grey lighten-1">
      ...
      <v-layout row>
        ...
        <v-flex md6>
          ...
          <v-btn v-else class="ma-2" tile large :color="car" icon @click="Send('bil','f0')">
            <v-icon>pause</v-icon>
          </v-btn>
        </v-flex>
        <v-flex md6>
  ...
  methods: {
    ...
    Connecting(connected) {
      ...
      if (connected == false) {
        ...
        this.car = "blue";
        this.Send("bil", this.clientId + " har anslutits.");
      }
    },
    ...
    Send(adress, message) {
      // console.log(message);
      this.client.publish(
        this.options.username + "/" + adress, //Exempel         "my.bjork@abbindustrigymnasium.se"+"/" + "bil",
        message
      );
